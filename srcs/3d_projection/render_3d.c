...
/*   Updated: 2023/01/25 21:27:51 by fbouanan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_directions(t_data *game, t_img *img, t_var *v, int i)
{
	if (game->wall[i].is_vertical)
	{
		...
		v->t_offsetx = (int)game->wall[i].vertiwallhity % game->g_v.title_size;
	}
	else
	{
		...
		v->t_offsetx = (int)game->wall[i].horiwallhitx % game->g_v.title_size;
	}
}
...
void	render_3d(t_data *game)
{
	...
	while (i < game->ray.num_rays)
	{
		...
		while (j < v.endp && j < WINDOW_HEIGHT)
		{
