...
/*   Updated: 2023/01/22 17:52:37 by fbouanan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	wall->yintercept = floor((game->p.y / game->g_v.title_size)) \
		* game->g_v.title_size;
	if (wall->is_down)
		wall->yintercept += game->g_v.title_size;
	wall->xintercept = game->p.x + \
		((wall->yintercept - game->p.y) / tan(game->ray.ray_angle));
	wall->ystep = game->g_v.title_size;
	if (wall->is_up)
		wall->ystep *= -1;
	wall->xstep = game->g_v.title_size / tan(game->ray.ray_angle);
...
bool	horizontal_ray(t_data *game, t_wall *wall)
{
	...
	while (next_horix >= 0 && next_horix < game->g_v.map_num_width
		&& next_horiy >= 0 && next_horiy < game->g_v.map_num_height)
	{
